...
void Create(){					// ====> Mendaftar menjadi member <===== //
	...
	printf("=========================================================================\n");
	printf("====================== LIST PENJUALAN NYC! ======================\n");
	printf("\nMasukkan Nama Customer \t\t\t: ");scanf("%[^\n]",&Penjualan.Nama);fflush(stdin);
	printf("Masukkan ID Customer \t\t\t: ");scanf("%d",&Penjualan.ID);fflush(stdin);
...
int Pembayaran_Kembalian (int total, int pembayaran){							// ====>  OPERASI PENGHITUNGAN JUMLAH YANG HARUS DIBAYAR  <==== //
    ...
    if (total != 0){
		if (pembayaran < total){
		    ...
		    fprintf(fpointer,"=================================================================\n");
		    fprintf(fpointer,"               ** TERIMA KASIH TELAH DATANG **\n");
	  		fprintf(fpointer,"=================================================================\n");
		    fclose(fpointer);
			if (point == 0){
		    selesai();}
...
int hitung_poin (int total_belanja){                   //menghitung point dengan 
    int poin = total_belanja / 200000;
    return poin;
}
int tukar_poin (int point){                            //menukar point untuk mendapatkan hadiah
	int pilih,pil2,pilihan;
    struct merchandise gantungan_kunci,payung,boneka;
    gantungan_kunci.point = 2;                         //point kunci ditetapkan menjadi 2
    payung.point=5;                                    //point payung ditetapkan menjadi 5
    boneka.point=7;                                    //point boneka ditetapkan menjadi 7
    ...
    do{
        ...
        new_line();
		if (pilih == 1){                                            //kondisi jika menukar point pada pilihan ke 1
        ...
        else if(pilih == 2){                                        //kondisi jika menukar point pada pilihan ke 2
            ...
            if(pil2 == 1){
                strcpy (payung.pilihan_warna,"Merah");              //mengcopy string Merah ke dalam variabel 
			}
            else if(pil2==2){
...
int Ppn(int total){                    //menghitung PPN setiap pembelian
...
int get_point(){							// ====>  OPERASI PEMANGGILAN POINT <- FILE  <==== //
    ...
    fscanf(fpointer,"Point anda sebesar : %d",&temp);    
    fclose(fpointer);
    return temp;
}
